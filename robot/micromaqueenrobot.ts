namespace microcode {
    ...
    class DFRobotMicroMaqqueenRobot extends robots.Robot {
        ...
        motorRun(left: number, right: number): void {
            if (left === 0 && right === 0) motorStop(Motors.All)
            else if (left === right) {
                const speed = left
        ...
        motorTurn(speed: number): void {
            if (speed === 0) motorStop(Motors.All)
            else {
                // TODO
            }
        }
    ...
    export const dfRobotMicroMaqueen = new RobotDriver(
        new DFRobotMicroMaqqueenRobot()
    )
}
